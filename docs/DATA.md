...
The core data asset is `policy-cce-data-v1.tar.gz`, available in the `data-v1` GitHub Release, separately from ordinary Git history. The archive contains one directory, `offline-inputs-v1/`. The authors' generated data are covered by the repository's [MIT License](../LICENSE).
...
Execution provenance metadata was detected in 3,958 files. This includes 3,356 files with project identifiers, 1,496 with executor/node metadata, 2,464 with storage metadata, and three with local-path metadata; these categories overlap. The authors have authorized public sharing with these original records preserved. A targeted scan is not a guarantee that a dataset contains no sensitive information.
